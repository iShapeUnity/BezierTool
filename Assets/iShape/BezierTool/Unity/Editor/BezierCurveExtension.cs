...
namespace iShape.BezierTool {
    ...
    public static class BezierCurveExtension {
        ...
        public static bool AddPointsToSelected(this BezierCurve curve) {
            var anchors = curve.anchors;
            int n = anchors.Count;
            bool isPrevSelected = anchors[0].IsSelectedPoint;
            var suitIndexes = new List<int>();
            for (int i = 1; i <= n; i++) {
                bool isCurrentSelected = anchors[i % n].IsSelectedPoint;
            ...
            for (int i = n - 1; i >= 0; i--) {
                ...
                var containerLineStart = spline.GetPoint(0.48f);
                var localPoint = spline.GetPoint(0.5f);
                var containerLineEnd = spline.GetPoint(0.52f);
                ...
                if (anchor.IsSelectedPoint) {
                    ...
                    if (curve.isClosed || i > 0) {
                        if (!anchor.IsPrevPinchAvailable) {
                            anchor.PrevPoint = sCurve.splines[(i - 1 + n) % n].GetPoint(0.8f);
                            result = true;
                        }
                        isNext = true;
                    }
                    ...
                    if (curve.isClosed || i < n - 1) {
                        if (!anchor.IsNextPinchAvailable) {
                            anchor.NextPoint = sCurve.splines[i].GetPoint(0.2f);
                            result = true;
                        }
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (anchor.IsSelectedPoint && prevAnchor.IsSelectedPoint) {
                    ...
                    if (!anchor.IsPrevPinchAvailable) {
                        ...
                        anchor.PrevPoint = spline.GetPoint(0.8f);
                        result = true;
                    }
                    ...
                    if (!prevAnchor.IsNextPinchAvailable) {
                        ...
                        prevAnchor.NextPoint = spline.GetPoint(0.2f);
                        result = true;
                    }
                }
        ...
        public static void AddPoint(this BezierCurve curve, Vector2 point) {
            ...
            var sCurve = new Curve(curve.anchors, isClosed: curve.isClosed);
        ...
        private static Anchor AddPoint(this BezierCurve curve, Vector2 localPoint, Vector2 containerLineStart, Vector2 containerLineEnd, int index) {
            var newAnchor = curve.anchors.AddPoint(localPoint, containerLineStart, containerLineEnd, index);
            return newAnchor;
        }

        private static void RemoveAnchor(this BezierCurve curve, int index) {
            if(index >= 0) {
                int n = curve.anchors.Count;
                int i = (index + n) % n;
                curve.anchors.RemoveAt(i);
            }
        }
    }
